(function() {
	...
	function regValue(id, i) {
		...
		case "send_to_name":
		case "send_from_name":
			flag = /^[a-zA-Z ]{1,20}$/.test(value.replace(/[\u0391-\uFFE5]/g,
					"nn"));
			id = "name";
			break;
	...
	;
	var inputs = document.getElementsByClassName("input"), id, hint = document
			.getElementsByClassName("hint"), index = 0;
	...
	regEvent(document.getElementById("login"), "click", function(e) {
		if (index !== 0) {
			e.preventDefault();
	...
	regEvent(document.getElementById("findBack"), "click", function(e) {
		if (index !== 0) {
			e.preventDefault();
			alert("您的输入有误，请检查并重新输入！");
			return false;
	...
	regEvent(document.getElementById("affirm"), "click", function(e) {
		if (index !== 0) {
			e.preventDefault();
			alert("您的输入有误，请检查并重新输入！");
			return false;
	...
	regEvent(document.getElementById("resetPassword"), "click", function(e) {
		if (index !== 0) {
			e.preventDefault();
			alert("您的输入有误，请检查并重新输入！");
			return false;
